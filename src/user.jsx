...
function App() {
  ...
  const parseUpdatedCode = (codeText) => {
    const files = {};
    const filePattern = /=== FILE: (.+?) ===([\s\S]*?)(?==== FILE:|=== END FILE ===|$)/g;
    let match;
 
    while ((match = filePattern.exec(codeText)) !== null) {
      const filePath = match[1].trim();
      let fileContent = match[2].trim();
      fileContent = fileContent.replace(/=== END FILE ===\s*$/, '').trim();
  ...
  const generatePreviewContent = () => {
    ...
    const jsFiles = Object.keys(parsedFiles).filter(file => 
      file.toLowerCase().endsWith('.js') || file.toLowerCase().endsWith('.jsx')
    );
    ...
    if (htmlFiles.length > 0) {
      ...
      const isReactCode = jsContent.includes('React') || jsContent.includes('import React') || 
                         jsContent.includes('jsx') || jsContent.includes('useState') || 
...
<body>
    ...
    <script type="text/babel">
        try {
            ...
            ${jsContent.replace(/import.*from.*['"];?/g, '// Import removed for preview')
                      .replace(/export default /g, 'const App = ')
                      .replace(/export \{[^}]*\}/g, '// Export removed for preview')}
            ...
            if (typeof App !== 'undefined') {
                ...
                document.getElementById('root').innerHTML = \`
                    <div style="padding: 40px; text-align: center; color: #666;">
                        ...
                        <p>Generated code contains React components but no default export found.</p>
                    </div>
                \`;
            }
        } catch (error) {
            ...
            document.getElementById('root').innerHTML = \`
                <div style="color: #dc2626; padding: 20px; border: 2px solid #dc2626; margin: 20px; border-radius: 8px;">
                    <h3>Preview Error</h3>
        ...
        setSelectedPreviewFile('React Preview');
      } else {
        previewContent = `<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="preview-container">
        ${jsContent.includes('document.') ? '' : '<div id="app"></div>'}
    </div>
    <script>
        try {
            ${jsContent}
        ...
        setSelectedPreviewFile('Web Preview');
      }
    } else {
      const fileList = Object.keys(parsedFiles).map(file => `
        <div style="margin: 10px 0; padding: 15px; border: 1px solid #ddd; border-radius: 8px; background: white;">
          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            <span style="background: #6366f1; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px; margin-right: 10px;">FILE</span>
            <strong style="color: #374151;">${file}</strong>
          </div>
          <pre style="background: #f8f9fa; padding: 15px; margin: 0; overflow: auto; max-height: 200px; font-size: 12px; border-radius: 4px;">${parsedFiles[file].substring(0, 1000)}${parsedFiles[file].length > 1000 ? '\n\n... (truncated)' : ''}</pre>
        </div>
      `).join('');
...
<head>
    ...
    <style>
        ...
        .file-list { max-width: 1000px; margin: 0 auto; }
...
<body>
    <div class="file-list">
        <div class="header">
            <h2 style="color: #374151; margin-bottom: 5px;">Generated Codebase</h2>
            <p style="color: #6b7280; margin: 0;">Preview of generated files and code structure</p>
        </div>
        ${fileList}
    </div>
</body>
  ...
  useEffect(() => {
    if (isPreviewOpen && (code || updatedCode)) {
  ...
  }, [code, updatedCode, isPreviewOpen]);
...
=== END FILE ===`;
  ...
  const WorkflowOverview = ({ workflow }) => {
    return (
      <div className="bg-gradient-to-r from-slate-50 to-blue-50 border-2 border-blue-200 rounded-xl p-6 shadow-lg">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <span className="bg-blue-600 text-white px-3 py-1 rounded-lg text-sm font-mono font-bold">
